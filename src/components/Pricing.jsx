        ...
        <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
          {publicPackages.filter(pkg => pkg.isActive).length > 0 ? (
            publicPackages.filter(pkg => pkg.isActive).map((pkg) => {
              ...
              return (
                <div key={pkg.id} className="bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 hover:border-[#b03500]/30 hover:shadow-xl hover:scale-105 transition-all flex flex-col shadow-sm w-[260px]">
                  <h3 className="text-lg sm:text-xl font-headline font-bold text-[#091d2e] mb-2">{content.name}</h3>
                  <div className="text-3xl sm:text-4xl font-extrabold text-[#ff5c16] mb-6">€{Number(pkg.price).toLocaleString('nl-NL')}</div>
                  {content.description && (
                  ...
                  <ul className="space-y-3 sm:space-y-4 mb-6 sm:mb-8 flex-grow">
                    {content.features && content.features.map((feature, index) => (
                      <li key={index} className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                        <span className="material-symbols-outlined text-[#ff5c16] text-[16px] sm:text-[18px]" style={{fontVariationSettings: "'FILL' 1"}}>check_circle</span> {feature}
                      </li>
                    ))}
                  </ul>
                  <a 
                    href="#contact"
                    className="block w-full py-3 rounded-xl border-2 border-[#ff5c16] text-[#ff5c16] font-bold hover:bg-[#ff5c16] hover:text-white transition-all text-sm sm:text-base text-center"
                  >
                    {currentLang === 'ar' 
                      ? `اختر ${content.name}` 
                      : `Kies ${content.name}`
